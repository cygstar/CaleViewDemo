...
object LunarUtil {
    ...
    private fun solarToInt(nY: Int, nM: Int, nD: Int): Long {
        var y = nY
        var m = nM
        m = (m + 9) % 12
        y -= m / 10
        return (365 * y + y / 4 - y / 100 + y / 400 + (m * 306 + 5) / 10 + (nD - 1)).toLong()
    }
    ...
    fun lunarToSolar(lunarYear: Int, lunarMonth: Int, lunarDay: Int, isLeap: Boolean): IntArray {
        ...
        var loop = leap
        if (!isLeap) {
            loop = if (lunarMonth <= leap || leap == 0) lunarMonth - 1 else lunarMonth
        }
